...
BEGIN
    DECLARE fecha DATE;
    SET fecha = CONCAT(anho, '-', LPAD(mes, 2, '0'), '-01');
    RETURN fecha;
END $$
DELIMITER;
...
DELIMITER $$
CREATE FUNCTION hallar_devengos(fecha_inicio DATE, fecha_fin DATE, id_empleado INT)
RETURNS DECIMAL(10, 2)
BEGIN
  DECLARE salario DECIMAL(10, 2);
  DECLARE deducciones DECIMAL(10, 2);
  DECLARE horas_trabajadas INT;
  DECLARE tarifa_horaria DECIMAL(10, 2);
  DECLARE devengos DECIMAL(10, 2);
  
  -- Obtener información del empleado
  SELECT salario, deducciones
  INTO salario, deducciones
  FROM empleados
  WHERE id = id_empleado;
  
  -- Obtener horas trabajadas durante el periodo de la nómina
  SELECT SUM(horas_trabajadas), tarifa_horaria
  INTO horas_trabajadas, tarifa_horaria
  FROM registros_horarios
  WHERE id_empleado = id_empleado
    AND fecha BETWEEN fecha_inicio AND fecha_fin;
  
  -- Calcular devengos
  SET devengos = salario + (horas_trabajadas * tarifa_horaria) - deducciones;
