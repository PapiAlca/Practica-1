# Práctica 2 Modelo Entidad Relación
## Apartado 1
### En nuestro centro educativo nos centramos en las personas, ya sean alumnos, profesores u otras. Nos gusta conocer a todas las personas por su nombre y guardamos celosamente los datos de contacto (dirección, tlf, email y DNI). Así mismo, de nuestros alumnos almacenamos el nombre y teléfono de sus padres, y de los profesores su número de la seguridad social y antigüedad en el cargo.
![Imagen 1:](/bbdd/img/ER1.png)
## Apartado 2
### Como somos un centro moderno permitimos la matriculación parcial por módulos de nuestros alumnos. Cada módulo tiene un código de curso y un número de horas.
![Imagen 2:](/bbdd/img/ER2.png)
## Apartado 3 
### Cada módulo será impartido por un profesor que será el encargado de hacer el seguimiento de faltas, retrasos y la evaluación de los progresos (nota) de cada alumno.
![Imagen 3:](/bbdd/img/ER3.png)
## Apartado 4
### Los alumnos estararán asociados a un grupo (p.e. DAW1) que pertenece a un ciclo formativo.
![Imagen 4:](/bbdd/img/ER4.png)
## Apartado 5
### Los grupos se componen de módulos y se imparten en aulas. Algunos módulos se imparten en grupos distintos.
![Imagen 5:](/bbdd/img/ER5.png)
## Apartado 6
### Los módulos pertenecen a alguno de los ciclos formativos (DAW, DAM, SMR, ASIR, etc.).
![Imagen 6:](/bbdd/img/ER6.png)
## Apartado 7
### Los grupos también pertenecen a los mismos ciclos.
![Imagen 7:](/bbdd/img/ER7.png)
## Apartado 8
### Los módulos rellenan horario total ocupando horas completas organizadas con código L1 para lunes a primera o X4 para miércoles a cuarta.
![Imagen 8:](/bbdd/img/ER8.png)
## Apartado 9
### Úne los apartados anteriores en un único modelo ER completo.
#### Como observacion he puesto las dos imágenes del apartado 9 separadas ya que no cabian en el draw.io, y como la primera imagen(relación) es algo más irrelevantes que el resto de relaciones que aparecen en la segunda imágen las pongo aparte.
![Imagen 9:](/bbdd/img/ER1.png)
![Imagen 10:](/bbdd/img/ER9.png)
